...
The spectrum of a quasar is shown in \cref{fi:quasar-spectrum}.
...
Lyman $\alpha$ absorption line systems can be counted to derive density of neutral hydrogen in the universe

find : about \SI{50}{\percent} of baryons are in the form of neutral hydrogen at $z=3$

$\rightarrow$ universe was already mostly reionized by then

Recall: reionization: baryons were mostly neutral after recombination $(z=1100)$ but were reionized by first astrophysical objects (Stars, galaxies, etc)


If universe was still neutral at $z=z_{q}$, quasar continuum would be fully absorbed at $\lambda < \lambda_{\alpha} \left(1+z_{9}\right) $.

$\rightarrow$ Can set limit on epoch of reionization: Gunn-Peterson Effect

$\rightarrow$ Find that reionization must have been completed by $z \sim 6$
>>>>>>> 606bc2ca57c4ed6f1a9ba40631c6823f0f8a9ba5
...
Consider the global rate of star formation:
\begin{align*}
	\dot{\rho}_\star(z)
	= \frac{\text {total gas mass turned into star at redshift } z}{\text{time } \times \text { volume}}
\end{align*}

It can be estimated:

Let $\avg{\dot{M}_\star}(L,z)$ be the star formation rate for galaxies with luminosity $L$ at redshift $z$.
...
\end{align*}
where $\phi(L, z)$ is the galaxy luminosity function, which is the number of galaxies formed per volume and luminosity.

\Cref{fig:star-formation-rate} shows that $\dot{\rho}_\star$ has dropped since $z \approx 2$

\begin{figure}
	\includegraphics[width=0.7\textwidth]{img/ch-05/star-formation-rate.pdf}
	\caption{The global star formation rate as a function of redshift, from \cite{galaxy-formation}. Different symbols correspond to different rest-frame wavelength ranges used to infer the star-formation rate, as indicated.}
...
How can we model baryonic structure formation and galaxy formation in particular?

How can one account for the diversity and statistical properties of galaxies?

$\rightarrow$ Describe basic framework for galaxy formation models

Still an unsolved problem.
There are many interlinked processes.


Cosmological model:
\begin{itemize}
	\item Background expansion
	\item Linear structure evolution
	\item Non-linear structure formation (DM halos)
\end{itemize}
$\rightarrow$ Add treatment of baryons
...
Feedback processes inject energy, mass or metals into the gas.
Feedback processes include
\begin{itemize}
	\item Supernovae explosions at the end of stellar evolution
	\item \Ac{smbh} in \ac{agn}
\end{itemize}

These processes can reheat cold gas and thus suppress star formation (negative feedback) or compress surrounding gas with shock waves and thus enhance star formation (positive feedback).
...
Galaxy formation is very complex and involves many intertwined processes.
Inflow and outflow of gas from and to the \ac{igm} also take place.


\subsection{Mergers}

Baryons are also effected by mergers, represented by merger trees.

\begin{itemize}
	\item If two halos of similar masses merge, violent relaxation quickly lead to a relaxed state for the new halo. Hot gas from the progenitors is shock-heated and settles into hydrostatic equilibrium with the DM halo potential. Central galaxies (if present) merge to a new central galaxy (likely Elliptical)
	\item If merging haloes have very different mass, the smaller halo orbits within main halo as a satellite until dynamical friction (kinetic energy is transferred from small to large object, making the small object sink towards the large one) and tidal effects dissolves it.
	\item In the limit of very small haloes merging into large halo, the accretion can be thought of as smooth accretion
\end{itemize}

\subsection{Dynamical evolution}

As satellite galaxies orbit within dark matter halos, tidal interactions and pressure from the gas in the halo (ram pressure) can remove dark matter, gas and stars from the satellite.
...
In particular, a bar instability may develop in spiral galaxies which can then buckle to create a pseudo-bulge. 
Bulges may therefore be remnants of early stages of galaxy formation or results from buckling of a bar.

\subsection{Chemical evolution}

Heavy elements (metals) are produced in stars and expelled by supernovae explosions and stellar winds into the \ac{ism}.

Importance for galaxy evolution:
\begin{itemize}
	\item Cooling efficiency depends strongly on metallicity (cooling more efficient for metal-enriched gas).
	\item Metallicity affects color and luminosity of stellar population in galaxies.
	\item Small particles of heavy elements called dust can absorb starlight and reradiate it in the IR (extinction).
\end{itemize}


\subsection{Stellar population synthesis}
The spectrum of galaxies is mostly a combination of the spectra of many stars. Given the star formation rate $\dot{M}_{*}(z)$ and the Initial Mass Function (IMF), the stellar population can be evolved using stellar evolution to derive a stellar synthesis spectrum.

The effect of gas emission and absorption, dust extinction and AGN emission also need to be modelled.
