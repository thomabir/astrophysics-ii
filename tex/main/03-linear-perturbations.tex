...
\begin{figure}
	...
	\caption{The power spectrum of the cosmic microwave background radiation temperature anisotropy in terms of the angular scale (or multipole moment). The measurements and the theoretical model are shown. The features, from left to right, TODO are the ISW Rise, the Sachs-Wolfe Plateau, the Acoustic Peak, and the Damping Tail.}
...
\end{align*}
so $\ell(\ell+1) C_\ell$ is constant, which is why the vertical axis in the plot was chosen in such a way.\sidenote{It is not exactly flat for $n=1$, because the Doppler term and the ISW term have been neglected here. TODO Diagram}
