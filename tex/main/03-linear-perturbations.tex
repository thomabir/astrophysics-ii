...
\end{align*}
where $V_u$ is the volume of a sufficiently large box. The fluid equation then becomes\sidenote{$\Grad \to i\vec{k}$ and $\Lap \to -k^2$}
\begin{align*}
...
Taking the moments of the Boltzmann equation one can show\sidenote{For more details, have a look at \cref{ssec:collisionless-eom}.} that one gets the same equations as the fluid equations: the continuity equation and the Euler equation, with the pressure gradient term replaced as follows:
\begin{align*}
	\frac{\Grad_i p}{\rho}
...
\end{align*}
where $k_J = a/c_s \sqrt{4 \pi G \bar{\rho}}$ is the Jeans wavenumber. From this equation, we see that the baryon perturbations are equal to the dark matter perturbations for small $k$. For large $k$, pressure becomes more important, and the baryon perturbations will be smaller and oscillate.\sidenote{Keep in mind that large $k$ means small length scales, and vice versa. When particles are close together, pressure becomes important.}
...
\begin{align*}
	g_{\mu \nu} = \bar{g}_{\mu\nu} + h_{\mu\nu},
\end{align*}
where $\bar{g}$ is the \ac{frw} metric, and $h$ is a again a small perturbation.
...
\begin{itemize}
	...
	\item $D(a)$ is the growth factor, normalized such that $D(a) = a$ in the matter era,\sidenote{We have derived $D(a) \propto a$ before, so we can do this.} and
...
\end{figure}
\newacronym{hdm}{HDM}{hot dark matter}

At small $k$, $T = 1$ as required.
\begin{itemize}
	\item In the \ac{cdm} model, $T(k)$ is suppressed at $k \gg k_\text{eq}$, as we have seen before. We find $T(k) \propto (k/k_\text{eq})^2 \ln(k/k_\text{eq})$.
	\item In the \ac{hdm} model\sidenote{For example, dark matter which is made up of neutrinos}, the perturbations are damped strongly due to free streaming when $k > k_\nu$.
	...
	\item In a mixed model\sidenote{\SI{70}{\percent} \ac{cdm} and \SI{30}{\percent} \ac{hdm}}, nothing very interesting happens.
	\item In \ac{cdm} + baryons (blue line), small oscillations occur around the \ac{cdm} curve.
\end{itemize}
...
\begin{itemize}
	\item With $\delta_H$, convenient to relate to the \ac{cmb} anisotropy power spectrum
...
\begin{itemize}
	...
	\item In the \textsc{scdm} model (no cosmological constant, compensated with larger $\Omega_m$), the turnover is at a higher $k$.
...
We look at the matter power spectrum for \textsc{Λcdm} in more detail in \cref{fig:power-lcdm}. 
\begin{itemize}
...
\newacronym{isw}{ISW}{integrated Sachs-Wolf term}
...
\end{align*}
so $\ell(\ell+1) C_\ell$ is constant, which is why the vertical axis in the plot was chosen in such a way.\sidenote{It is not exactly flat for $n=1$, because the Doppler term and the \ac{isw} term have been neglected here.}
...
%The results are shown in \cref{tab:constraints}.
...
In \cref{fig:sdss-power-spectrum}, the power spectrum found by the \textsc{sdss} is shown.
