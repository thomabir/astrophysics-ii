...
\end{align*}
with the comoving number density $n(\vec{x}, t) = \int \dd{^3p} f(\vec{x}, \vec{p}, t)$.
Some moments are
\begin{itemize}
	\item the density $\rho(\vec{x}, t)$, which is obtained with $Q = m$, where $m$ is the mass of a particle, and
	\item the bulk velocity $\langle v_i \rangle$, which is obtained with $Q = v_i = p_i/(m n)$.
\end{itemize}

We take moments of the Boltzmann equation. One can show\sidenote{For more details, have a look at \cref{ssec:collisionless-eom}.} that one gets the same equations as the fluid equations: the continuity equation and the Euler equation, with the pressure gradient term replaced as follows:
\begin{align*}
	\frac{\Grad_i p}{\rho}
