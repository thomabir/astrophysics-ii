...
\begin{marginfigure}
	\centering
	\includegraphics[width=\textwidth]{img/ionization.png}
...
We had defined the particle horizon
\begin{align*}
...
\end{align*}
We look at the deviation of $\Omega$ form $1$:
\begin{align*}
...
\begin{align*}
	...
	&= \frac{\Omega_\text{init}^{-1} - 1}{\Omega_\text{eq}^{-1} -1}
	\frac{\Omega_\text{eq}^{-1}-1}{\Omega_0^{-1}-1}\\
	&= 
	\left( \frac{a_\text{init}}{a_\text{eq}} \right)^2 
	\left( \frac{a_\text{eq}}{a_0} \right)\\
	&=	\left( \frac{T_\text{init}}{T_\text{eq}} \right)^2 
	\left( \frac{T_\text{eq}}{T_0} \right) \text{  TODO probably wrong }
\end{align*}
...
The solid line is the past light cone $\chi_p$, and the dashed lines are the forward light cone with ($\chi_f$) and without ($\chi_f'$) inflation.

The horizon problem can be stated mathematically as $\chi_f' < \chi_p$: Regions which are observable today to have similar temperatures were apparently not in causal contact at last scattering. A solution is to increase $\chi_f'$ through a period of accelerated expansion called inflation.
