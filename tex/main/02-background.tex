...
\end{align*}
which is the maximal comoving distance that a photon could have travelled from the Big Bang ($t=0$) until time $t$. Let's look at the size of the horizon at decoupling time, where photons scattered for the last time. This corresponds to a redshift $z_\text{dec} \approx 1100$. Evaluating the integral yields
\begin{align*}
...
We have observationally determined $\Omega_0 \approx 1$, this means we have a flat geometry. Let's look at how $\Omega$ varies with time. For this, we can use the Friemann equation:
\begin{align*}
...
\begin{align*}
	...
	\end{cases}
\end{align*}
Consider an initial time $t_i$ in the radiation era ($t_i < t_eq$). Then
\begin{align*}
