...
\glsxtrnewsymbol[description={comoving radius}]{chi}{$\chi$}
\glsxtrnewsymbol[description={scale factor}]{a}{$a$}
...
\subsection{Redshift}
\glsxtrnewsymbol[description={redshift}]{z}{$z$}
...
\glsxtrnewsymbol[description={Hubble constant, Hubble parameter}]{H}{$H$}
\glsxtrnewsymbol[description={dimensionless Hubble parameter}]{h}{$h$}
...
\label{sec:Friedmann}
\glsxtrnewsymbol[description={density}]{rho}{$\rho$}
\glsxtrnewsymbol[description={matter density}]{rho-matter}{$\rho_m$}
\glsxtrnewsymbol[description={radiation density}]{rho-radiation}{$\rho_r$}
\glsxtrnewsymbol[description={cosmological constant}]{Lambda}{$\Lambda$}
\glsxtrnewsymbol[description={density parameter}]{Omega}{$\Omega$}

The Friedmann equations\sidenote{Usually, the 1st Friedmann equation is just called \enquote{Friedmann equation}, and when we need the 2nd one, we explicitly call it \enquote{2nd Friedmann equation}.} are derived by plugging the \ac{frw} metric into Einstein's equations and assuming a uniform and ideal fluid to find the stress tensor (see \cref{sec:general-relativity}):
\begin{align*}
...
The Friedmann equation is a differential equation for the scale factor $a(t)$ and it is also possible to derive it from Newtonian calculations.\sidenote{When deriving the Friedmann equation with Newtonian gravity, we are essentially getting the correct result, but starting from the wrong assumptions. Interestingly, it still works somehow \textellipsis}

The critical density is defined as
\begin{align*}
	\rho_\text{crit}(t) = \frac{3 H(t)^2}{8 \pi G}.
\end{align*}
When the actual density of the universe is equal to the critical density, the universe is flat and Euclidean.\sidenote{In earlier models, where there was no cosmological constant included, the critical density was interpreted as the decisive point between an expanding and a collapsing universe. It was thought that when the actual density of the universe was higher than the critical density, the universe would slowly start collapsing.}
Today, the critical density is about five hydrogen atoms per cubic metre, or one galaxy per \si{\mega\parsec\cubed}.

Density parameters \gls{Omega} give a more convenient description of the universe, and are usually much easier to remember than densities:
\begin{itemize}
...
\section{Solutions}
\glsxtrnewsymbol[description={equation of state parameter}]{w}{$w$}
...
The results can be plugged into the convenient form of the Friedmann equation (\ref{eq:friedmann-nice}):
\begin{align*}
	\frac{H}{H_0} 
...
The evolution of the universe with different density parameters is shown in \cref{fig:evolution}.
...
\glsxtrnewsymbol[description={angular-diameter distance}]{d-A}{$d_A$}
\glsxtrnewsymbol[description={luminosity distance}]{d-L}{$d_L$}
...
\glsxtrnewsymbol[description={comoving horizon}]{chi-h}{$\chi_h$}
...
\glsxtrnewsymbol[description={interaction rate}]{Gamma}{$\Gamma$}
...
In \cref{tab:thermal-history} and \cref{fig:big-bang}, an overview of the thermal history of the universe is given.
...
\begin{table*}
	...
	\describe{\textsc{Qcd} phase transition}{The universe has cooled sufficiently such that hadrons (baryons and mesons) can form.} & \SI{e-5}{\second} & \num{e12} & \SI{200}{\MeV} \newline \SI{3e12}{\kelvin}\\
	...
	\describe{Surface of last scattering}{The number density of charged particles has decreased enough for photons to decouple. These photons form the \textsc{cmb}.} &  &  & \\
...
\glsxtrnewsymbol[description={phase space distribution function}]{f}{$f(\vec{x}, \vec{p}, t)$}
...
T
he neutrons are unstable to beta decay $n \leftrightarrow p + e + \bar{\nu}_e$, and thus the neutron-to-proton ratio continues to decrease.
...
The ionization fraction $X_e$ is the ratio between the number density of free electrons $n_e$ and the number density of baryons ($n_b = n_p + n_H$):
\begin{align*}
...
According to \textsc{Λcdm}, the energy content of the universe today is made up of radiation, matter (consists of baryons and dark matter), and dark energy.
