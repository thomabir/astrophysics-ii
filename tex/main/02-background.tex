...
\begin{figure}
	\centering
	\includegraphics[width=\textwidth]{img/inflation.png}
...
Inflation requires vacuum energy, which is realized as a scalar field $\phi(\vec{x}, t)$, called the \emph{inflaton}. The potential of the scalar field is sketched in \cref{fig:inflaton-potential}.
...
We need a \emph{slow roll condition}, which impedes the field from changing too quickly:
\begin{align*}
	\dot{\phi}^2 \ll V(\phi)
\end{align*}
There are several models, some of which are called old inflation, new inflation, and chaotic inflation. There are many other models proposed, indicating that inflation is still an ongoing field of research.
