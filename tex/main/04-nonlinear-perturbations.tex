...
Small haloes form first, and merge to form larger haloes.

A convenient description of this process are halo merger trees.
We consider a halo with mass $M_0$ at time $t_0$.
We can represent the merging history of this halo using such a merging tree.
% TODO figure
The thickness of a branch corresponds to the mass of the halo at that time.
We see that several smaller progenitor haloes merge into larger ones as time time goes on.

We can generate such merger trees with two approaches:
\begin{itemize}
	\item track haloes in numerical simulations
	\item analytical methods with extended Press-Schechter (EPS) approach
\end{itemize}
...
Main progenitor.
This is the main branch of the merger tree.
The mass of the main progenitor $M_\text{main}(t)$ can be obtained from the merger tree.
...
Right panel:
Each line is a different main progenitor mass.
More massive haloes assemble later, but quicker.

\subsection{Characteristic scales}

We define more quantities to reduce the complexity of the merger trees.
...
The \emph{halo merger rate} is the rate at which a halo of mass $M$ changes its mass by $\Delta M$, which can be expressed as a probability:
\begin{align*}
	\Prob(\Delta M \, | \, M, t) \dd{\ln \Delta M} \dd{\ln t}.
\end{align*}

The \emph{halo survival time} $t_s$ is the time it takes a halo of mass $M$ to merge to form a halo of mass $2 M$.
...
and $\bar{N}_h$ is the mean number of such haloes in the volume $\dd{V}$.

In the linear regime, one can show (using PSE), that
\begin{align*}
	\delta_h(\vec{x}, t)
