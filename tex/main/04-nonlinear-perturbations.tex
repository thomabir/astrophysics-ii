...
For the spherical collapse model, we found that for regions with a smooth density field satisfying
\begin{align*}
	\delta_s(\vec{x}, R, t) > \delta_c \approx 1.69,
...
We now to calculate the probability of collapse.
...
For $M < \num{1e12} M_\sol$, $b_h < 1$, which is called an antibias.
Generally, the more massive the haloes, the more strongly clustered they are.
...
From simulations, we know that density profiles of haloes are well fit by an NFW model (\cref{ssec:spherical-models}),
% TODO reference
\begin{align*}
	\rho(r)
...
Let $a_1 \geq a_2 \geq a_3$ be the main axis of the ellipsoid, and let $s = a_3/a_1$, which is a measure of flattening of the halo.
...
\subsection{Halo substructure}

% TODO figure from Millennium Run

There are subhaloes inside haloes, which are remnants of progenitor haloes that have merged to form larger haloes.

Let $M$ be the total mass of a halo, and $m$ the mass of a subhalo.
The mass distribution of small haloes inside larger haloes is called the \emph{subhalo mass function},
% This is a good fit to simulations
\begin{align*}
	\dv{n}{h(m/M)}
	\approx 
...
\begin{align*}
	...
	\int m \dv{M}{m} \dd{m}.
\end{align*}
The expression in the integral is the total mass of the subhaloes, and it is divided by the total mass of the halo. It is called the \emph{mean subhalo mass fraction}, which is the mass of a halo which is made up of subhaloes.
